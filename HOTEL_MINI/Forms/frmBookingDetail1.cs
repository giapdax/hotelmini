...
namespace HOTEL_MINI.Forms
{
    public partial class frmBookingDetail1 : Form
    {
        ...
        private static decimal? FindRate(IEnumerable<RoomPricing> list, string pricingType)
        {
            ...
            return p != null ? (decimal?)p.Price : null;
        }
        ...
        private decimal CalcCostByPlan(SelectedRoomWithTime item)
        {
            ...
            if (plan.PricingType.Equals("Hourly", StringComparison.OrdinalIgnoreCase))
            {
                int hours = CeilToInt((co - ci).TotalHours);
                return unit * hours;
            }
            if (plan.PricingType.Equals("Daily", StringComparison.OrdinalIgnoreCase))
            {
                int days = CeilToInt((co - ci).TotalDays);
                if (days < 1) days = 1;
                return unit * days;
            }
            if (plan.PricingType.Equals("Weekly", StringComparison.OrdinalIgnoreCase))
            {
                int weeks = CeilToInt((co - ci).TotalDays / 7.0);
                if (weeks < 1) weeks = 1;
                return unit * weeks;
            }
            if (plan.PricingType.Equals("Nightly", StringComparison.OrdinalIgnoreCase))
            {
                return unit;
            }
            return 0m;
        ...
        {
            var m = dgvHotelServices.CurrentRow?.DataBoundItem as ServiceMenuItem;
            if (m == null)
            {
            ...
            var roomsPicked = GetCheckedRooms();
            if (roomsPicked.Count == 0)
            {
            ...
            foreach (var room in roomsPicked)
            {
